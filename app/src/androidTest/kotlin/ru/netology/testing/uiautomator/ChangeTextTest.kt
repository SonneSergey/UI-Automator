...
//    }
    ...
    fun testEmptyInputDoesNotChangeText() {
        ...
        val afterClickText = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals("Text should not change on empty input", initialText, afterClickText)
    }

    @Test
    fun testChangeTextInNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        val inputField = device.findObject(By.res(packageName, "userInput"))
        val activityButton = device.findObject(By.res(packageName, "buttonActivity"))

        val textToEnter = "Netology UI Automator"
        inputField.text = textToEnter
        activityButton.click()

        device.wait(Until.hasObject(By.res(packageName, "text")), TIMEOUT)
        val result = device.findObject(By.res(packageName, "text")).text

        assertEquals("Text should match in new activity", textToEnter, result)
    }
